...
proc newOptimizer*(level: OptimizationLevel = olFull): Optimizer =
  ...
  let flags =
    ...
    of olFull:
      {
        okConstantFolding, okBooleanSimplification, okArithmeticSimplification,
        okComparisonSimplification, okTypeCheckSimplification,
        okConditionalSimplification, okRemoveGrouping,
      }

  Optimizer(level: level, flags: flags, enabled: level != olNone)

proc setOptimizationFlag*(
    optimizer: var Optimizer, flag: OptimizationKind, enabled: bool
) =
...
proc optimizeArithmetic*(
    optimizer: Optimizer, op: TokenKind, left, right: Expression, pos: Position
): Expression =
  ## Optimize arithmetic binary operations with constant operands
  if not optimizer.isEnabled(okConstantFolding):
    return nil

  if left.kind == ekValue and right.kind == ekValue and left.value.kind == vkNumber and
...
proc optimizeBoolean*(
  ...
  if left.kind == ekValue and right.kind == ekValue and left.value.kind == vkBool and
...
proc optimizeComparison*(
  ...
  if left.kind == ekValue and right.kind == ekValue and left.value.kind == vkNumber and
      right.value.kind == vkNumber and left.value.number.kind != nkComplex and
...
proc optimizeUnaryMinus*(
    optimizer: Optimizer, operand: Expression, pos: Position
): Expression =
...
proc optimizeConditional*(
    optimizer: Optimizer, branches: seq[Branch], elseBranch: Expression
): Expression =
...
proc optimizeTypeCheck*(
    optimizer: Optimizer, right: Expression, pos: Position
): Expression =
...
proc optimizeUnaryOp*(
    optimizer: Optimizer, op: TokenKind, operand: Expression, pos: Position
): Expression =
...
# =============================================================================
proc optimizeGrouping*(
    optimizer: Optimizer, expr: Expression, pos: Position
): Expression =
