  ...
  of 1:
    ...
    if values[0].kind == vkVector:
      ...
      resultSeq.generator = Generator(
        atEnd: proc(): bool = index >= vec.len,
        next: proc(peek: bool = false): Value =
          if index >= vec.len:
            raise newSequenceExhaustedError("Sequence exhausted: attempted to access beyond the end of sequence derived from vector of length " & $vec.len)
          result = vec[index]
          if not peek:
            inc index
      )
      ...
      resultSeq.generator = Generator(
        atEnd: proc(): bool = false,  # Never ends
        next: proc(peek: bool = false): Value =
      ...
      resultSeq.generator = Generator(
        atEnd: proc(): bool = false,  # Never ends
        next: proc(peek: bool = false): Value = value
      )
  
  of 2:
    # Case 3: Number and function (finite sequence up to the given number)
    if values[0].kind != vkNumber or values[0].number.kind != nkInt:
      raise newTypeError("sequence expects an integer value for the sequence length, but got " & 
        (if values[0].kind == vkNumber: "a " & $values[0].number.kind & " number" else: "a " & $values[0].kind))
    
    if values[1].kind != vkFunction and values[1].kind != vkNativeFunc:
      raise newTypeError("sequence expects a function as the second argument, but got a " & $values[1].kind)
    
    let limit = values[0].number.iValue
    if limit <= 0:
      raise newInvalidArgumentError("sequence expects a positive integer for the sequence length, but got " & $limit)
    
    var counter = 0
    let function = values[1]
    
    resultSeq.generator = Generator(
      atEnd: proc(): bool = counter >= limit,
      next: proc(peek: bool = false): Value =
        if counter >= limit:
          raise newSequenceExhaustedError("Sequence exhausted: attempted to access beyond the end of finite sequence of length " & $limit)
    ...
    raise newInvalidArgumentError("sequence expects 1 or 2 arguments (value/function or length+function), but got " & $values.len & " arguments")
...
proc skip*(sequence: Value, n: Value): Value =
  ...
  if sequence.kind != vkSeq:
    raise newTypeError("skip requires a sequence as the first argument, but got a " & $sequence.kind)
  if n.kind != vkNumber or n.number.kind != nkInt:
    raise newTypeError("skip requires an integer as the second argument, but got " & 
      (if n.kind == vkNumber: "a " & $n.number.kind & " number" else: "a " & $n.kind))
  if n.number.iValue < 0:
    raise newInvalidArgumentError(
      "skip requires a non-negative integer as the second argument, but got " & $n.number.iValue
    )
...
proc hasNext*(sequence: Value): Value =
  ...
  if sequence.kind != vkSeq:
    raise newTypeError("hasNext requires a sequence as the argument, but got a " & $sequence.kind)
...
proc next*(sequence: Value): Value =
  ...
  if sequence.kind != vkSeq:
    raise newTypeError("next requires a sequence as the argument, but got a " & $sequence.kind)
  
  if sequence.sequence.generator.atEnd():
    raise newSequenceExhaustedError("Cannot get next element: sequence has been exhausted")
      ...
      for transformer in sequence.transformers:
        if transformer.kind == tkMap:
          ...
          if condition.kind != vkBool:
            raise newTypeError("Filter function must return a boolean, but got a " & $condition.kind)
...
proc collect*(sequence: Value): Value =
  ...
  if sequence.kind != vkSeq:
    raise newTypeError("collect expects a sequence as argument, but got a " & $sequence.kind)
